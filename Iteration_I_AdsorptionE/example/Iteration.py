...
class iteration():
    ...
    def analyze_data(self,origin_data,new_data,Num,sigma_f=1,alpha=0.2,length_scale=1,max_cycle=500,std=True,random_state=False,threshold_std=0.7,threshold_dE=1,kk=1,outliers=5):
        
        #new_data is from feature construction
        #origin_data is structures
        ...
        for i in range(max_cycle):
            ...
            if std is True:
                if MaxStd_testset > threshold_std:
                    management, sigma = iteration.gpr(self,X,training_set_variables,training_set_observations,sigma_f,alpha,length_scale,test_set_variables,test_set_observations,run_std,
                    ...
                    MAE_trainingset, MAE_testset, std_Tr = iteration.plot_deltaE(self,management)
                    ...
                    #MAE_total = iteration.MAE_X(self,Y,management)
                    iteration.plot_algorithm(self,SIGMA,threshold_std)
                    ...
                    bad_data = iteration.bad_data_index(self, MaxPredStd)
                    print (bad_data)
                    if outliers == 1:
                        Num_outliers_percent = iteration.Num_outlier(self,Y, management,threshold_dE)  # dE > threshold_dE
                    if outliers == 2:
                        Num_outliers_percent = iteration.Num_outlier_percent(self,Y,management,threshold_dE)  # dE > threshold_dE
                        print ("outliers == %s is for the all data, the definition of outlier is dE > threshold_dE!" % outliers)
                    if outliers == 3:
                        Num_outliers_percent = iteration.New_Num_outliers_percent(self,Y,management,threshold_std)  # (dft-ml)/std > 2
                        print ("outliers == %s is for the all data!, the definition of outlier is dE/sigma_ml > 2!" % outliers)
                    if outliers == 4:
                        Num_outliers_percent = iteration.New_Num_outliers_percent_trainingset(self,management,threshold_std)  # (dft-ml)/std > 2
                        print ("outliers == %s is for the training set, the definition of outlier is dE/sigma_ml > 2!" % outliers)
                    if outliers == 5:
                        Num_outliers_percent = iteration.New_Num_outliers_percent_testset(self,management,threshold_std)  # (dft-ml)/std > 2
                        print ("outliers == %s is for the test set, the definition of outlier is dE/sigma_ml > 2!" % outliers)
                    iteration.save_variables(self,training_set_variables,training_set_observations)
                    break
                ...
                if MaxdE_testset > threshold_dE:
                    ...
                    iteration.plot_algorithm_dE(self,SIGMA,threshold_dE)
                    Num_outliers_percent = []                 # no definition    
                    
                    break
        
        return i, MaxdE_testset, MAE_trainingset, MAE_testset, std_Tr, Num_outliers_percent
        
        
    def gpr(self,X,training_set_variables,training_set_observations,sigma_f,alpha,length_scale,test_set_variables,test_set_observations,run_std,random_state,threshold_std,threshold_dE,training_set_AdsSlabSite,test_set_AdsSlabSite,kk):
        #kernel_1 = 1**2 * RBF(length_scale=17,length_scale_bounds=(1e-5,1e5)) + 1 * WhiteKernel(noise_level=0.1,noise_level_bounds=(1e-5,1))
        #kernel_rbf = 1**2 * RBF(length_scale=17,length_scale_bounds=(1e-5,1e5)) * C(1,(1e-3,1e3))
        #constant_kernel = ConstantKernel(constant_value=alpha, constant_value_bounds=(1e-05, 1e5))
        #kernel_1 = constant_kernel * (sigma_f)**2 * RBF(length_scale=length_scale,length_scale_bounds=(1e-5,1e5)) # 0.5
        kernel_1 = (sigma_f)**2 * RBF(length_scale=length_scale,length_scale_bounds=(1e-5,1e5)) # 0.5
        #kernel_1 = constant_kernel * (sigma_f)**2 * RBF(length_scale=length_scale,length_scale_bounds=(1e-5,1e5)) + 1**2 * WhiteKernel(noise_level=alpha,noise_level_bounds=(1e-5,1))
        #kernel_1 = constant_kernel * (sigma_f)**2 * Matern(length_scale=length_scale,length_scale_bounds=(1e-5,1e5),nu=3)   # [1.5,2.5]
        #kernel_2 = 1**2 * RationalQuadratic(length_scale=length_scale,alpha=alpha)
        #kernel_1 = constant_kernel * (sigma_f)**2 * ExpSineSquared(length_scale=length_scale,periodicity=10)
        #kernel_2 = DotProduct(sigma_0=length_scale)
        kernel = kernel_1
        model = GaussianProcessRegressor(kernel=kernel,alpha=alpha,n_restarts_optimizer=10,random_state=42)  # 
        ...
        print ("Posterior (kernel : %s) \nLog_Likelihood: %.3f" % (model.kernel_, model.log_marginal_likelihood(model.kernel_.theta)))
    ...
    def plot_deltaE(self,management):
        ...
        df_test['deltaE_testset'] = deltaE_testset
        df_test.to_csv('deltaE_testset.csv',index=False)
        ...
        ax.set_xlabel('E$_{DFT}$ - E$_{ML}$ (eV)',fontsize=20)
        ax.set_ylabel('Normalized frequency',fontsize=20)
        ax.tick_params(axis='both',direction='in',labelsize=18)
        ax.set_xlim(-1.5,1.5)
        ax.text(0.25,1.25,"MAE = %s eV" % round(MAE_testset,3),fontsize=16)
        ...
        ax.annotate('$\sigma$ = %s' % round(std,2),xy=(-0.5,norm.pdf(-0.5,mu,std)),xytext=(-1.25,0.75),arrowprops=dict(facecolor='black',width=0.4,headwidth=6),fontsize=16)  # 6
        fig.tight_layout()
        ...
        from scipy.stats import norm
        mu, std_Tr = norm.fit(deltaE_trainingset)
        
        return MAE_trainingset, MAE_testset, std_Tr
    ...
    def save_AdsSlabSite(self,col_var,management):
        ...
        AdsSlabSite_trainingset.insert(loc=len(col_var),column='Eml',value=predict_observations_trainingset)
        ...
        AdsSlabSite_testset.insert(loc=len(col_var),column='Eml',value=predict_observations_testset)
    ...
    def MAE_X(self,Y, management):
        y_ml = management['Y_predicted']
        delE = [abs(Y[i] - y_ml[i]) for i in range(len(y_ml))]
        MAE_total = sum(delE)/len(delE)
        return MAE_total
        
    def plot_algorithm(self,SIGMA,threshold_std):
        N = len(SIGMA)
        fig, ax = plt.subplots()
        ax.tick_params(axis='both',direction='in',labelsize=14)
        ...
        ax.plot(range(N), MaxY,'o-', color='r') # one point
        ax.set_xlabel('Iteration #',fontsize=16)
        ax.set_ylabel('Predicted error (eV)', fontsize=16)
        

        ax.legend(['MPE = %s eV' % threshold_std], loc='best',fontsize=14)
        fig.tight_layout()
        plt.savefig('ML_algorithm_1.pdf')
        plt.savefig('ML_algorithm_1.png')
        
    def Num_outlier(self,Y, management,threshold_dE):
        training_set_observations = management['training_set_observations']
        predict_observations_trainingset = management['predict_observations_trainingset']
        ...
        y_ml = management['Y_predicted']
        delE = [abs(Y[i] - y_ml[i]) for i in range(len(y_ml))]
        outliers = [e for e in delE if e > threshold_dE ]   #####  here the outlier is abs(delta_E) > threshold_dE.
        Num_outliers = len(outliers) 
        
        return Num_outliers        

    def Num_outlier_percent(self,Y, management,threshold_dE):
        training_set_observations = management['training_set_observations']
        predict_observations_trainingset = management['predict_observations_trainingset']
        delE_trainingset = [abs(training_set_observations[k] - predict_observations_trainingset[k]) for k in range(len(predict_observations_trainingset))]
        test_set_observations = management['test_set_observations']
        ...
        y_ml = management['Y_predicted']
        delE = [abs(Y[i] - y_ml[i]) for i in range(len(y_ml))]
        new_outliers = [e for e in delE if e > threshold_dE]   #here the outlier is abs(delta_E) > threshold_dE.
    ...
    def New_Num_outliers_percent(self,Y,management,threshold_std):
        E_ml = management['Y_predicted']
        Std = management['std']
        outliers = []
        delE = [abs(Y[i] - E_ml[i])/threshold_std for i in range(len(E_ml))]
        for de in delE:
            if de > 2:
                outliers.append(de)
        ...
        print ("-------------------------------------")
        print ("Num_outliers_percent [|E_dft - E_ml|/Std > 2]: %s" % Num_outliers_percent)
        return Num_outliers_percent

    def New_Num_outliers_percent_trainingset(self,management,threshold_std):
        training_set_observations = management['training_set_observations']
        predict_observations_trainingset = management['predict_observations_trainingset']
        #std_trainingset = management['std_trainingset']
        outliers = []
        delE = [abs(training_set_observations[i] - predict_observations_trainingset[i])/threshold_std for i in range(len(training_set_observations))]
        for de in delE:
            if de > 2:
                outliers.append(de)
    ...
    def New_Num_outliers_percent_testset(self,management,threshold_std):
        test_set_observations = management['test_set_observations']
        predict_observations_testset = management['predict_observations_testset']
        test_set_AdsSlabSite = management['test_set_AdsSlabSite']
        #std_testset = management['std_testset']
        ...
        #--------
        delE = [abs(test_set_observations[i] - predict_observations_testset[i])/threshold_std for i in range(len(test_set_observations))]
        ...
        iteration.plot_outliers_testset(self,dft_good, ml_good, dft_bad, ml_bad,threshold_std)
        return Num_outliers_percent

    def plot_outliers_testset(self,dft_good, ml_good, dft_bad, ml_bad,threshold_std):
        ...
        y_down = t - 2 * threshold_std
        y_up = t + 2 * threshold_std
    ...
    def plot_algorithm_dE(self,SIGMA,threshold_dE):
        N = len(SIGMA)
        fig, ax = plt.subplots()
        ax.tick_params(axis='both',direction='in',labelsize=14)
        for n in range(N):
        ...
        yy = 50 * [threshold_dE]
        ax.plot(xx,yy,linestyle='--',color='black',linewidth=2)
        ax.legend(['$Max_{_}\DetalE$ = %s' % threshold_dE], loc='best',fontsize=14)
        fig.tight_layout()
        plt.savefig('ML_algorithm_1.pdf')
        plt.savefig('ML_algorithm_1.png')     

...
class iteration:
    def __init__(self,origin_data,new_data,Num,alpha=0.2,length_scale=1,n_restarts_optimizer=10,
                 ...
                 threshold_dE=1,kk=1,outliers=5,ntimes=2,outlier_identifier=False):
        """
        This class can identify the outlier that was selected by the algorithm.
        """
        ...
        self.ntimes = ntimes
        self.outlier_identifier = outlier_identifier
    def analyze_data(self):
        ...
        features_columns = list(df.columns.to_numpy())
        features_columns.remove(features_columns[-1])
        ...
        StruOfAdd = [] 
        MaxPredStd = []
        if self.outlier_identifier == True:
            Bad_trainingset_ASS = []
            Bad_trainingset_variable = []                                
            ...
            if self.std is True:
                if MaxStd_testset > self.threshold_std:
                    ...
                    for elem in elements:
                        ...
                        """
                        if self.outlier_identifier == True:
                            E_dft = test_set_observations[indx]
                            E_ml = predict_observations_testset[indx]
                            x = self.if_outlier(E_ml, E_dft)
                            if x > self.ntimes:
                                Bad_trainingset_variable.append(elem)
                                Bad_trainingset_ASS.append(test_set_AdsSlabSite[indx])
                    ...
                    if self.outlier_identifier == True:
                        self.save_outlier_trainingset(Bad_trainingset_ASS,col_var,Bad_trainingset_variable,features_columns)
    ...
    def if_outlier(self,E_ml, E_dft):
        diff = abs(E_ml - E_dft)
        x = round(diff/self.threshold_std,2)
        return x

    def save_outlier_trainingset(self,Bad_trainingset_ASS,Col_var,Bad_trainingset_variable,features_columns):
        ass = pd.DataFrame(Bad_trainingset_ASS,columns=Col_var)
        data = pd.DataFrame(Bad_trainingset_variable,columns=features_columns)
        ass.to_csv('Selected_Bad_ASS.csv',index=False)
        data.to_csv('Selected_Bad_variable.csv',index=False)
    ...
    def plot_deltaE(self,management):
        ...
        ax.tick_params(axis='both',direction='in',labelsize=14)
        ax.set_xlim(-1.5,1.5)
        ax.text(0.5,1,"MAE = %s eV" % round(MAE_testset,3),fontsize=14)
        #---------------------PDF
        from scipy.stats import norm
        ...
        ax.annotate('$\sigma$ = %s' % round(std,2),xy=(-0.4,norm.pdf(-0.4,mu,std)),xytext=(-1.25,0.75),arrowprops=dict(facecolor='black',width=0.4,headwidth=6),fontsize=16)  # 6        
        #-------------------

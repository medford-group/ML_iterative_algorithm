...
class Run_Label_Encode:
    ...
    def __init__(self,atomType_chemPot,N_int,ref_atomType_chemPot=None,contour3d=True,
    ...
    def run_data_encode(self):
        ...
        name = self.str_name()
        N = self.N_int
        data = pickle.load(open('matrix_structures_%s_%s.pickle' % (name,N),'rb'))
        ...
        if len(self.atomType_chemPot) == 1:
            ...
            with open ('label_matrix_%s_%s.pickle' % (name,N),'wb') as data_label:
                pickle.dump(label_matrix,data_label)
            
            energy_matrix = pickle.load(open('matrix_energies_%s_%s.pickle' % (name,N),'rb'))
    ...
    def plot_2d_pd(self,label_matrix):
        ...
        N = self.N_int
        fig, ax = plt.subplots()
        p = ax.pcolor(muB,muA,label_matrix,cmap=plt.cm.get_cmap('RdBu_r'))  # Dark2,   # cmap=plt.cm.get_cmap('RdBu_r')
        fig.colorbar(p)
